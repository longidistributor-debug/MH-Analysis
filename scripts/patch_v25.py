...
new='''    override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{
        when(intent?.action){
            ...
            "JS_LIVE_TICK"->{val s=intent.getStringExtra("symbol")?:return START_STICKY;val tf=intent.getStringExtra("timeframe")?:"1m";val price=intent.getDoubleExtra("c",0.0);if(price<=0)return START_STICKY;val c=Candle(intent.getLongExtra("t",0L),price,price,price,price,intent.getDoubleExtra("v",0.0));thread(name="mh-visible-live-tick"){processLifecycleTick(s,c);val key="$s|$tf";val now=System.currentTimeMillis();val prior=lastEval[key]?:0L;if(now-prior>=5_000L){lastEval[key]=now;validatePendingOnTimeframe(s,tf)}};return START_STICKY}
        }
...
# Native socket header patch is handled by patch_v25_socket.py so this script stays compatible with the current hub implementation.
