...
# v37: timeframe-aware confluence analysis + rich diagnostics.
# Keep v36 lifecycle/alarm recovery intact. This patch only strengthens analysis,
# dynamic validity, and what the user sees when there is no trade.
...
# Replace the old single-family gate with a multi-pillar gate. Existing setup
# families (BOS/CHoCH, liquidity, FVG/OB, S/R, trendline/wick/video layers, etc.)
# are preserved; this adds a fallback when several independent pillars agree.
...
new_gate=family_line+'''        val tfm=tfMinutes(timeframe)
        ...
        // Independent evidence pillars. A trade can qualify through a named
        // setup family OR through broad confluence across at least four pillars.
...
# Make fallback family wording truthful when a signal qualified through broad
# confluence instead of one named pattern.
s=s.replace('else->"established trend pullback"}', 'else->"multi-factor confluence"}', 1)

# Add permanent major S/R to the setup explanation so every accepted signal
# states the structural map it was evaluated against.
...
if old_setup not in s:
    raise SystemExit('v37 setupReason anchor not found')
s=s.replace(old_setup,new_setup,1)

# Replace setupCheck with a deeper, timeframe-aware live thesis test. No fixed
# candle expiry is introduced; this is purely structure/momentum/liquidity based.
...
new_check='''    fun setupCheck(s:Signal,c:List<Candle>):SetupCheck{
        ...
        live+=if(s.direction=="BUY")"EMA20 ${fmt(e20)} / EMA50 ${fmt(e50)}" else "EMA20 ${fmt(e20)} / EMA50 ${fmt(e50)}"
        live+="RSI ${one(rr)}"
...
# Replace the generic one-line no-signal text with a real selected-timeframe
# diagnostic. This intentionally varies with market state, not a canned template.
...
new_no_signal='''    fun noSignalReason(symbol:String,timeframe:String,c:List<Candle>):String{
        ...
        val prior=c.takeLast((lookback+2).coerceAtMost(c.size)).dropLast(1)
        val priorHigh=prior.maxOf{it.h};val priorLow=prior.minOf{it.l}
        val bosBull=last.c>priorHigh+a*.03;val bosBear=last.c<priorLow-a*.03
        val majors=majorSupportResistance(c,a);val support=majors.first;val resistance=majors.second
        val lm=liquidityMap(c,a)
        val sweepBull=last.l<support-a*.03&&last.c>support
        val sweepBear=last.h>resistance+a*.03&&last.c<resistance
        ...
        val bestIr=listOfNotNull(irBull,irBear).maxByOrNull{it.continuationStrength}

        val trend=when{
            e20>e50&&s20>s50&&last.c>e150->"BULLISH"
            e20<e50&&s20<s50&&last.c<e150->"BEARISH"
            else->"MIXED / TRANSITION"
        }
        val structure=when{
            bosBull->"Bullish BOS confirmed above ${fmt(priorHigh)}"
            bosBear->"Bearish BOS confirmed below ${fmt(priorLow)}"
            sweepBull->"Sell-side liquidity swept at ${fmt(support)} and reclaimed"
            sweepBear->"Buy-side liquidity swept at ${fmt(resistance)} and rejected"
            last.c>(priorHigh+priorLow)/2->"Price is in upper half of ${fmt(priorLow)}–${fmt(priorHigh)} range; no clean BOS yet"
            else->"Price is in lower half of ${fmt(priorLow)}–${fmt(priorHigh)} range; no clean BOS yet"
        }
        val momentum=when{
            rr>=55&&macd>macdPrev->"Bullish pressure • RSI ${one(rr)} • MACD rising"
            rr<=45&&macd<macdPrev->"Bearish pressure • RSI ${one(rr)} • MACD falling"
            else->"Neutral/conflicted • RSI ${one(rr)} • MACD ${if(macd>macdPrev)"rising" else "falling"}"
        }
        ...
        var fvgText="No active recent FVG near current price"
        val fw=c.takeLast(min(30,c.size))
        for(i in 2 until fw.size){
            if(fw[i].l>fw[i-2].h){val lo=fw[i-2].h;val hi=fw[i].l;if(abs(last.c-(lo+hi)/2)<=a*2.5)fvgText="Bullish FVG ${fmt(lo)}–${fmt(hi)}"}
            if(fw[i].h<fw[i-2].l){val lo=fw[i].h;val hi=fw[i-2].l;if(abs(last.c-(lo+hi)/2)<=a*2.5)fvgText="Bearish FVG ${fmt(lo)}–${fmt(hi)}"}
        }
        val bullOb=c.takeLast(min(16,c.size)).dropLast(1).lastOrNull{it.c<it.o}
        val bearOb=c.takeLast(min(16,c.size)).dropLast(1).lastOrNull{it.c>it.o}
        val obText="Bull OB ${bullOb?.let{fmt((it.o+it.c)/2)}?:"-"} • Bear OB ${bearOb?.let{fmt((it.o+it.c)/2)}?:"-"}"
        val liqText=when{
            sweepBull->"Sell-side pool at ${fmt(support)} was swept/reclaimed"
            sweepBear->"Buy-side pool at ${fmt(resistance)} was swept/rejected"
            else->"${lm.supports.size} support liquidity pool(s), ${lm.resistances.size} resistance pool(s) • nearest major S/R ${fmt(support)} / ${fmt(resistance)}"
        }
        ...
        val waiting=when{
            trend=="BULLISH"&&!bosBull->"Need bullish BOS/CHoCH above ${fmt(priorHigh)} plus a defended retest, liquidity reclaim, or displacement confirmation."
            trend=="BEARISH"&&!bosBear->"Need bearish BOS/CHoCH below ${fmt(priorLow)} plus a rejected retest, liquidity sweep, or displacement confirmation."
            trend.startsWith("MIXED")->"Need directional separation: a confirmed BOS/CHoCH away from the range, then retest/liquidity confirmation. Current evidence conflicts."
            bosBull&&rr<50->"Bullish break exists, but momentum has not confirmed it yet; wait for RSI/MACD recovery or a clean retest hold."
            bosBear&&rr>50->"Bearish break exists, but momentum has not confirmed it yet; wait for RSI/MACD weakness or a clean retest rejection."
            else->"The structure is developing, but independent trend + structure + liquidity/zone + momentum pillars have not aligned strongly enough yet."
        }
...
# Add permanent major support/resistance helper before liquidityMap.
...
helper='''    private fun majorSupportResistance(c:List<Candle>,a:Double):Pair<Double,Double>{
        ...
        for(i in 2 until w.size-2){
            val x=w[i]
            if(x.l<=w[i-1].l&&x.l<=w[i-2].l&&x.l<=w[i+1].l&&x.l<=w[i+2].l)lows+=x.l
            if(x.h>=w[i-1].h&&x.h>=w[i-2].h&&x.h>=w[i+1].h&&x.h>=w[i+2].h)highs+=x.h
        }
        val support=lows.filter{it<=last+a*.15}.maxOrNull()?:w.minOf{it.l}
        val resistance=highs.filter{it>=last-a*.15}.minOrNull()?:w.maxOf{it.h}
        return support to resistance
    }

'''
if anchor not in s: raise SystemExit('v37 liquidityMap anchor not found')
s=s.replace(anchor,helper+anchor,1)
engine_path=p
p.write_text(s)

# -----------------------------------------------------------------------------
# SignalStore: pending signals can expire inside the current candle if a new,
# materially stronger setup has formed. Do NOT auto-create a new trade; NEW
# ANALYZE remains the explicit action for a fresh signal.
# -----------------------------------------------------------------------------
...
new_eval='''    @Synchronized fun evaluateLiveStructure(c:Context,symbol:String,timeframe:String,candles:List<Candle>,at:Long=System.currentTimeMillis()):List<ProcessEvent>{
        ...
        // Re-score the current candle/history from cache. If market structure
        // has already formed a materially stronger different setup, expire the
        // old pending signal immediately. A replacement is NOT auto-traded;
        // the user can press NEW ANALYZE to accept a new signal deliberately.
...
# -----------------------------------------------------------------------------
# Main UI: no-signal diagnostic already contains arrows and multiple lines.
# -----------------------------------------------------------------------------
...
# ACTIVE conclusion should surface any live structural warning if present.
...
# -----------------------------------------------------------------------------
# Floating UI: show the same selected-timeframe no-trade diagnostic after an
# explicit NEW ANALYZE instead of silently reverting to generic text.
# -----------------------------------------------------------------------------
p=Path('app/src/main/java/com/mh/analysis/OverlayService.kt')
s=p.read_text()
needle='''SignalStore.evaluate(this,symbol,period,data);val s=AnalysisEngine.analyze(symbol,period,data);if(s==null){showState();return@post};'''
repl='''SignalStore.evaluate(this,symbol,period,data);val s=AnalysisEngine.analyze(symbol,period,data);if(s==null){status?.text=AnalysisEngine.noSignalReason(symbol,period,data);overlay(SignalStore.displayState(this,symbol,period));return@post};'''
if needle not in s: raise SystemExit('v37 overlay analyze anchor not found')
...
print('v37 timeframe-aware confluence analysis patch applied')
