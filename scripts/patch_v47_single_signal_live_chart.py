...
# ANALYZE/RE-EVALUATE remain FCS/API driven; the visual chart is not.

# -----------------------------------------------------------------------------
# SignalStore: one active signal per symbol across all timeframes.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Main activity: real TradingView owns live candles; latest completed ANALYZE
# clears older timeframe signals for that symbol.
# -----------------------------------------------------------------------------
...
<style>html,body{margin:0;padding:0;width:100%;height:100%;background:#131722;overflow:hidden;font-family:Arial,sans-serif}#tvroot{position:absolute;inset:0;background:#131722}#widget{position:absolute;inset:0;z-index:1}#state{position:absolute;left:7px;bottom:6px;z-index:4;color:#7f8999;font-size:7px;background:rgba(19,23,34,.60);padding:2px 4px;border-radius:3px;pointer-events:none}</style></head>
<body><div id="tvroot"><div id="widget"></div><div id="state">LIVE TRADINGVIEW</div></div><script src="https://s3.tradingview.com/tv.js"></script><script>
let tvWidget=null,chartApi=null,token=0,signal=null,analysis=null,shapeIds=[];
...
function getApi(){try{if(tvWidget&&typeof tvWidget.activeChart==='function')return tvWidget.activeChart();if(tvWidget&&typeof tvWidget.chart==='function')return tvWidget.chart()}catch(e){}return null}
function ready(t){if(t!==token)return;chartApi=getApi();redraw()}
...
function loadTradingView(symbol,period){token++;chartApi=null;shapeIds=[];build(symbol,period,token)}
function setAnalysisLevels(raw){analysis=parse(raw);redraw()}function clearAnalysisLevels(){analysis=null;redraw()}function setSignalCard(raw){signal=parse(raw);redraw()}function clearSignalCard(){signal=null;redraw()}
...
print('v47 real live TradingView + one-current-signal patch applied')
