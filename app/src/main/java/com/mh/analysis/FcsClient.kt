...
object FcsClient {
    ...
    private val sourceCache=mutableMapOf<String,Cache>()
    private var lastNetworkAt=0L
    private const val CACHE_MS=20_000L
    private const val SOURCE_CACHE_MS=45_000L
    private const val MIN_GAP_MS=2_500L

    @Synchronized
    fun history(accessKey:String,symbol:String,period:String,length:Int=220,force:Boolean=false):Pair<List<Candle>,Int>{
        val key="${symbol.uppercase()}|${period.lowercase()}|$length"
        val now=System.currentTimeMillis()
        val hit=cache[key]
        if(!force && hit!=null && now-hit.at<CACHE_MS) return hit.candles to 0

        val plan=periodPlan(period)
        val sourceLength=(length*plan.multiplier+plan.multiplier*6).coerceAtMost(1500)
        val sourceKey="${symbol.uppercase()}|${plan.sourcePeriod.lowercase()}|$sourceLength"
        val sourceHit=sourceCache[sourceKey]
        val raw=if(!force && sourceHit!=null && now-sourceHit.at<SOURCE_CACHE_MS){
            sourceHit.candles to 0
        }else{
            val wait=max(0L,MIN_GAP_MS-(System.currentTimeMillis()-lastNetworkAt))
            if(wait>0)Thread.sleep(wait)
            val fetched=when(symbol.uppercase()){
                "XAUUSD" -> fetch("forex",accessKey,"XAUUSD",plan.sourcePeriod,sourceLength,"commodity")
                else -> try{fetch("crypto",accessKey,"BINANCE:BTCUSDT",plan.sourcePeriod,sourceLength,"crypto")}
                catch(_:Exception){fetch("crypto",accessKey,"BTCUSDT",plan.sourcePeriod,sourceLength,"crypto")}
            }
            lastNetworkAt=System.currentTimeMillis()
            sourceCache[sourceKey]=Cache(lastNetworkAt,fetched.first,fetched.second)
            fetched
        }
        ...
        cache[key]=Cache(System.currentTimeMillis(),candles,raw.second)
        return candles to raw.second
    }
