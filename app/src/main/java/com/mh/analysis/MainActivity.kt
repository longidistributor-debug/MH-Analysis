...
class MainActivity:Activity(){
    ...
    private fun newAnalyze(){
        ...
        if(s==null){status.text="NO NEW TRADE\n${AnalysisEngine.noSignalReason(symbol,period,candles)}";showSignalOverlay(currentDisplayedSignal());return}
        val duplicate=SignalStore.findDuplicate(this,s);if(duplicate!=null){status.text="NO NEW TRADE — SAME SETUP ALREADY ${if(duplicate.state=="ACTIVE")"TRIGGERED / ACTIVE" else duplicate.state}\nExisting ${duplicate.signal.direction} setup around ${price(duplicate.signal.entry)} is still the same idea. Duplicate not recorded.";showSignalOverlay(duplicate);return}
    ...
    private fun showExisting(){
        if(!::status.isInitialized)return;val a=currentDisplayedSignal();if(a==null){status.text="NO CURRENT SETUP\nPress NEW ANALYZE for a fresh assessment.";if(::chart.isInitialized)chart.evaluateJavascript("setSignal(null)",null);return};val s=a.signal
        val life=if(a.state=="ACTIVE")"TRIGGERED / OPEN — removed from pending alarm state and tracked toward TP1/SL." else "PENDING — background monitor will trigger, expire/miss, or invalidate it when market data changes."
    ...
    private fun alarmAndShow(){val pending=SignalStore.loadActive(this,symbol,period);if(pending!=null){AlarmStore.addSaved(this,pending.signal);Toast.makeText(this,"Saved to Alarm list. Turn ON to enable sound alerts.",Toast.LENGTH_SHORT).show()}else Toast.makeText(this,"No pending setup. Triggered trades are tracked in Records.",Toast.LENGTH_SHORT).show();showAlarms()}
    private fun showAlarms(){
        ...
        fun refresh(){listBox.removeAllViews();val day=spinner.selectedItem?.toString()?:days.first();val entries=AlarmStore.forDay(this,day).filter{it.status !in setOf("TRIGGERED","EXPIRED")};if(entries.isEmpty())listBox.addView(TextView(this).apply{text="No pending alarms for this day. Triggered/expired items move to Records.";setTextColor(Color.BLACK);setPadding(0,dp(12),0,dp(12))});entries.forEach{a->val block=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(0,dp(7),0,dp(7))};block.addView(TextView(this).apply{text="${a.symbol} ${a.timeframe} • ${a.direction}\nEntry ${price(a.entry)} • ${a.status}";setTextColor(Color.BLACK)});val controls=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL};controls.addView(Button(this).apply{text=if(a.status=="ARMED")"OFF" else "ON";setOnClickListener{val on=a.status!="ARMED";AlarmStore.setEnabled(this@MainActivity,a.id,on);if(on)startStateService();refresh()}},LinearLayout.LayoutParams(0,dp(46),1f));controls.addView(Button(this).apply{text="DELETE";setOnClickListener{AlarmStore.delete(this@MainActivity,a.id);refresh()}},LinearLayout.LayoutParams(0,dp(46),1f));block.addView(controls);listBox.addView(block)}}
