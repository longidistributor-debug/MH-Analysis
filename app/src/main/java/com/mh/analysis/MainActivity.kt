...
class MainActivity:Activity(),LiveSocketHub.Listener{
    ...
    private fun ui():View{
        ...
        sp.onItemSelectedListener=object:AdapterView.OnItemSelectedListener{override fun onItemSelected(p:AdapterView<*>?,v:View?,pos:Int,id:Long){val np=periods[pos];if(np==period)return;period=np;prefs.edit().putString("period",period).apply();cancelPendingAnalyze();showCachedOrSwitching();if(!FcsClient.hasUsableHistory(symbol,period,60))load(false)};override fun onNothingSelected(p:AdapterView<*>?){}}
    ...
    private fun showCachedOrSwitching(){
        ...
        candles=emptyList();loadedSymbol="";loadedPeriod="";if(chartReady)chart.evaluateJavascript("clearChart();showMessage(${JSONObject.quote("WAITING FOR FIRST $symbol • $period CANDLE • LIVE CONNECTION CONTINUES")})",null);status.text="$symbol • $period\nLIVE FEED CONNECTING • HISTORY LOADS ONLY IF NEEDED"
    }
    ...
    private fun updateSocketUi(){val saved=savedSocketKey().isNotBlank();if(saved&&!editingSocket){socketInput.setText("");socketInput.visibility=View.GONE;socketStatus.text=if(LiveSocketHub.isConnected())"● LIVE STREAM CONNECTED" else "● LIVE STREAM KEY SAVED • AUTO-RECONNECT";socketButton.text="UPDATE LIVE KEY"}else{socketInput.visibility=View.VISIBLE;socketInput.setText("");socketStatus.text=if(saved)"ENTER NEW LIVE KEY • CURRENT STAYS ACTIVE UNTIL SAVE" else "LIVE STREAM KEY REQUIRED ONCE";socketButton.text=if(saved)"SAVE NEW LIVE KEY" else "SAVE LIVE KEY"}}
    ...
    private fun switchPair(s:String){if(symbol==s)return;symbol=s;prefs.edit().putString("symbol",s).apply();cancelPendingAnalyze();showCachedOrSwitching();if(!FcsClient.hasUsableHistory(symbol,period,60))load(false)}

    private fun load(force:Boolean){
        if(!chartReady)return;val k=savedKey();if(k.isBlank()){status.text="$symbol • $period\nLIVE STREAM CAN RUN • HISTORY KEY REQUIRED FOR FULL ANALYSIS";return};if(busy){queuedLoad=true;queuedForce=queuedForce||force;return};busy=true;val reqSymbol=symbol;val reqPeriod=period
        thread{try{val(_,credits)=FcsClient.seedForPeriod(k,reqSymbol,reqPeriod,force);runOnUiThread{if(credits>0)addUsage(credits);calls.text="History calls: ${usage()}/500";if(reqSymbol==symbol&&reqPeriod==period){showCachedOrSwitching();consumePendingAnalyze(reqSymbol,reqPeriod)};busy=false;drainQueuedLoad()}}catch(e:Exception){runOnUiThread{if(reqSymbol==symbol&&reqPeriod==period){showCachedOrSwitching();if(candles.isEmpty())status.text="$symbol • $period\n${e.message}"};busy=false;drainQueuedLoad()}}}
    }
    ...
    private fun consumePendingAnalyze(reqSymbol:String,reqPeriod:String){val token=pendingAnalyzeToken?:return;if(reqSymbol!=symbol||reqPeriod!=period||token!=analyzeGeneration)return;if(candles.size<60)return;pendingAnalyzeToken=null;performAnalysis()}
    private fun performAnalysis(){
        if(candles.size<60||loadedSymbol!=symbol||loadedPeriod!=period){status.text="$symbol • $period\nNOT ENOUGH CANDLES FOR RELIABLE ANALYSIS YET";return}
