...
class AlarmService:Service(){
            ...
            try{
                ...
                if(last!=null){
                    armed.filter{it.symbol==symbol}.forEach{a->
                        if(last.l<=a.entry&&last.h>=a.entry){
                ...
                updateService("Watching ${AlarmStore.armed(this).size} armed signal(s) • no fixed time expiry")
            }catch(_:Exception){}
    ...
    private fun fireThreeCycles(a:AlarmEntry){
        ...
        for(i in 1..3){val ring=runCatching{RingtoneManager.getRingtone(this,uri)}.getOrNull();runCatching{ring?.play()};try{Thread.sleep(2500)}catch(_:InterruptedException){};runCatching{ring?.stop()};if(i<3)try{Thread.sleep(1800)}catch(_:InterruptedException){}}
    }

    private fun createChannels(){if(Build.VERSION.SDK_INT>=26){val nm=getSystemService(NOTIFICATION_SERVICE) as NotificationManager;nm.createNotificationChannel(NotificationChannel("mh_alarm_service","MH Signal Watch",NotificationManager.IMPORTANCE_LOW));nm.createNotificationChannel(NotificationChannel("mh_alarm_alert","MH Signal Alerts",NotificationManager.IMPORTANCE_HIGH).apply{enableVibration(true)})}}
