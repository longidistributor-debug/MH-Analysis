...
class AlarmService:Service(),LiveSocketHub.Listener{
    ...
    private val statusTick=object:Runnable{override fun run(){if(running){updateService("${if(LiveSocketHub.isConnected())"LIVE SOCKET" else "CONNECTING"} • Pending ${SignalStore.pendingSignals(this@AlarmService).size} • Open ${SignalStore.openTrades(this@AlarmService).size} • Armed ${AlarmStore.armed(this@AlarmService).size}");h.postDelayed(this,10_000)}}}

    override fun onBind(intent:Intent?):IBinder?=null
    override fun onCreate(){
        super.onCreate();createChannels();startForeground(311,serviceNotification("Starting continuous WebSocket market engine"));LiveSocketHub.addListener(this)
    ...
    private fun bootstrapNeededHistory(){
        ...
        val selected=prefs.getString("symbol","XAUUSD")?:"XAUUSD"
        val needed=(listOf(selected)+SignalStore.pendingSignals(this).map{it.signal.symbol}+SignalStore.openTrades(this).map{it.signal.symbol}).distinct()
        for(s in needed){if(!running)return;if(!FcsClient.hasUsableHistory(s,"1h",100))runCatching{FcsClient.bootstrap(rest,s,false)}}
    }
    ...
    private fun processLifecycleTick(symbol:String,last:Candle){
        val oneMin=FcsClient.peek(symbol,"1m",220)?:return;if(oneMin.size<20)return
    ...
    private fun serviceNotification(text:String):Notification{val stop=Intent(this,AlarmService::class.java).apply{action="STOP"};val pi=PendingIntent.getService(this,91,stop,PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE);return builder("mh_alarm_service_v19").setSmallIcon(android.R.drawable.ic_lock_idle_alarm).setContentTitle("MH Analysis • WebSocket live engine").setContentText(text).setOngoing(true).addAction(Notification.Action.Builder(android.R.drawable.ic_menu_close_clear_cancel,"Stop",pi).build()).build()}
