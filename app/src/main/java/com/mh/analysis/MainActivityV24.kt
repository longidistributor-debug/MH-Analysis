...
class MainActivityV24:Activity(),LiveSocketHub.Listener{
    ...
    override fun onCreate(b:Bundle?){
        ...
        if(savedSocketKey().isNotBlank())startStateService()
    }
    override fun onResume(){super.onResume();LiveSocketHub.addListener(this);val k=savedSocketKey();if(k.isNotBlank()){LiveSocketHub.start(this,k);startStateService()};restoreChart();showExisting()}
    ...
    override fun onSocketState(state:String){runOnUiThread{if(::socketStatus.isInitialized)socketStatus.text="● $state";if(chartReady)chart.evaluateJavascript("setNativeState(${JSONObject.quote(state)})",null)}}
    override fun onLiveCandle(s:String,timeframe:String,c:Candle){
        if(s!=symbol||timeframe!=period)return
        runOnUiThread{
            FcsClient.peek(symbol,period,220)?.let{candles=it;loadedSymbol=symbol;loadedPeriod=period}
            if(chartReady){val j=JSONObject().put("t",c.t).put("o",c.o).put("h",c.h).put("l",c.l).put("c",c.c).put("v",c.v);chart.evaluateJavascript("pushNativeLive(${JSONObject.quote(s)},${JSONObject.quote(timeframe)},${JSONObject.quote(j.toString())})",null)}
            showExisting()
        }
    }
    ...
    private fun buildUi():View{
        ...
        root.addView(section("LIVE MARKET CHART"));val cc=card();chart=WebView(this).apply{settings.javaScriptEnabled=true;settings.domStorageEnabled=true;setBackgroundColor(Color.BLACK);webViewClient=object:WebViewClient(){override fun onPageFinished(v:WebView?,u:String?){chartReady=true;switchVisibleFeed();restoreChart();chart.evaluateJavascript("setNativeState(${JSONObject.quote(LiveSocketHub.currentState())})",null)}};loadUrl("file:///android_asset/chart_v27.html")};cc.addView(chart,LinearLayout.LayoutParams(-1,dp(560)));root.addView(cc)
    ...
    private fun analyzeNow(){
        val key=savedHistoryKey();if(key.isBlank()){status.text="SAVE ANALYSIS/HISTORY KEY ONCE • LIVE WEBSOCKET CHART CONTINUES";return}
    ...
    private fun updateKeyUi(){val h=savedHistoryKey().isNotBlank();historyInput.visibility=if(h&&!editHistory)View.GONE else View.VISIBLE;historyStatus.text=if(h&&!editHistory)"● ANALYSIS HISTORY KEY SAVED" else "ANALYSIS/HISTORY KEY";historyButton.text=if(h&&!editHistory)"UPDATE HISTORY KEY" else "SAVE HISTORY KEY";val s=savedSocketKey().isNotBlank();socketInput.visibility=if(s&&!editSocket)View.GONE else View.VISIBLE;socketStatus.text=if(s&&!editSocket)"● LIVE WEBSOCKET KEY SAVED • CONNECTING" else "LIVE WEBSOCKET KEY";socketButton.text=if(s&&!editSocket)"UPDATE LIVE KEY" else "SAVE LIVE KEY"}
    ...
    private fun saveSocketKey(){if(savedSocketKey().isNotBlank()&&!editSocket){editSocket=true;updateKeyUi();return};val x=socketInput.text.toString().trim();if(x.isBlank())return;prefs.edit().putString("socket_api_key",x).apply();editSocket=false;socketInput.setText("");updateKeyUi();LiveSocketHub.start(this,x);startStateService();Toast.makeText(this,"WebSocket key saved • native live feed starting",Toast.LENGTH_LONG).show()}
