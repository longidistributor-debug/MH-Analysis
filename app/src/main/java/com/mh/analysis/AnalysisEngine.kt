...
object AnalysisEngine {
    ...
    fun analyze(symbol:String,timeframe:String,c:List<Candle>):Signal?{
        ...
        val pressureBull=last.c>last.o&&body/range>=.58&&last.c>=last.h-range*.18
        val pressureBear=last.c<last.o&&body/range>=.58&&last.c<=last.l+range*.18
        val displacementBull=last.c-last.o>a*.50&&pressureBull
        val displacementBear=last.o-last.c>a*.50&&pressureBear
        ...
        if(r>=55)b(6,"RSI bullish ${one(r)}")else if(r<=45)s(6,"RSI bearish ${one(r)}")
        ...
        val bullContinuation=trendBull&&(displacementBull||bosBull)&&(fvgBull||bullObMid!=null||nearSupport)
        val bearContinuation=trendBear&&(displacementBear||bosBear)&&(fvgBear||bearObMid!=null||nearResistance)
        val bullChoch=chochBull&&(stopHuntBull||rsiBullDiv||displacementBull)
        val bearChoch=chochBear&&(stopHuntBear||rsiBearDiv||displacementBear)
        ...
        if(family<1||win<48||sep<10)return null
        ...
        val validEntries=candidates.filter{(_,v)->
            ...
            val directional=if(dir=="BUY")v<last.c+a*.08 else v>last.c-a*.08
            val notAlreadyPassed=if(dir=="BUY")v>last.l-a*.20 else v<last.h+a*.20
            directional&&notAlreadyPassed&&dist<=1.8
        ...
        val maxRiskAtr=when{tf<=5->.95;tf<=15->1.08;tf<=30->1.22;tf<=120->1.45;tf<=360->1.60;else->1.75}
        ...
        val tp1Atr=when{tf<=5->.90;tf<=15->1.05;tf<=30->1.18;tf<=120->1.48;tf<=360->1.72;else->1.95}
        val tp2Atr=when{tf<=5->1.25;tf<=15->1.50;tf<=30->1.70;tf<=120->2.10;tf<=360->2.45;else->2.80}
        ...
        val score=(60+sep+(family*6)).coerceIn(60,96)
        val reasons=(if(dir=="BUY")br else sr).sortedByDescending{it.first}.take(10).map{"${it.second} (+${it.first})"}
        val validityReason="No candle-count expiry. The setup remains pending only while its live structure, momentum, liquidity context and volatility remain valid. It expires immediately when that thesis breaks or abnormal volatility invalidates the pending entry."
        val setupReason="$dir $familyName selected. Evidence score $win vs $lose; entry uses $entrySource at ${fmt(entry)}. The engine does not require every indicator at once; it requires one complete setup family plus supporting confluence."
    ...
    fun noSignalReason(symbol:String,timeframe:String,c:List<Candle>):String{
        ...
        return "NO VALID TRADE / WAIT\nNo complete high-quality setup family is confirmed on $symbol $timeframe. EMA20 ${if(e20>e50)"above" else "below"} EMA50, RSI ${one(rr)}. The engine waits for a real liquidity reversal, breakout/retest, CHoCH reversal or trend-continuation setup instead of forcing a trade."
    }
    ...
    private fun tfMinutes(tf:String)=when(tf.lowercase(Locale.US)){"3m"->3;"5m"->5;"10m"->10;"15m"->15;"30m"->30;"1h"->60;"2h"->120;"4h"->240;"6h"->360;"12h"->720;"1d","1day"->1440;else->15}
