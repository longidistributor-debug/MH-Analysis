(function (global) {
    ...
    class FCSClient {
        ...
        connect() {
            ...
            return new Promise((resolve, reject) => {
                ...
                this.socket.onmessage = (event) => {
                    let data;
                    try { data = JSON.parse(isNode ? event.data.toString() : event.data); } catch (e) { return; }
                    if (data.type === 'ping') { this.send({ type: 'pong', timestamp: Date.now() }); return; }
                    else if (data.type === 'welcome') {
                        this.isConnected = true; this.countreconnects = 0; this.intentionalDisconnect = false; this.visibilityDisconnectTime = null;
                        this.rejoinAll(); this.startHeartbeat();
                        ...
                        if (data.short && data.short === 'joined_room' && data.symbol && data.timeframe) {
                ...
                this.socket.onclose = (event) => {
                    ...
                    if (!this.manualClose) {
                        ...
                        if (this.visibilityDisconnectTime) {
                            const delay = this.focusTimeout, elapsed = Date.now() - this.visibilityDisconnectTime;
                            if (elapsed >= delay) { this.visibilityDisconnectTime = null; return; }
                        }
                        this.countreconnects++;
                        if(this.countreconnects > this.reconnectlimit){ if (this.reconnectlimit > 0) {} return; }
        ...
        startHeartbeat() { if (!this.socket) return; this.stopHeartbeat(); this.heartbeat = setInterval(() => { if (this.socket && this.socket.readyState === WebSocketImpl.OPEN) { if (isNode) this.socket.ping(); } this.send({ type: 'ping', timestamp: Date.now() }); }, 25000); }
